...
{
	unsigned int		dCBWSignature;		// 0x43425355
	unsigned int		dCBWTag;		// random, same as receive 
	unsigned int		dCBWDataTransferLength;	// send length or want receive length
	unsigned char		bmCBWFlags;		// 00 send, 80 receive
	unsigned char		bCBWLUN;		// 00 
	unsigned char		bCBWCBLength;		// length of CBWCB 
	unsigned char		CBWCB[16];		// command data
} UMS_SCSI_CBW/* __attribute__((packed))*/;

//receive pack
typedef struct UMS_SCSI_CSW
{
	unsigned int		dCSWSignature;		// 0x53425355
	unsigned int		dCSWTag;		// random, same as send
	unsigned int		dCSWDataResidue;	// 00
	unsigned char		bCSWStatus;		// the status of command, 0 success, 1 fault
} UMS_SCSI_CSW;
#pragma pack()
