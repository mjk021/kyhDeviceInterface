...
namespace std{
  ...
  //in vs2008, min(unsigned int, unsigned int) complie error

  inline unsigned int min(unsigned int __a, unsigned int __b) {
    return __b < __a ? __b : __a;
  }
  inline unsigned int max(unsigned int __a, unsigned int __b) {
    return  __a < __b ? __b : __a;
  }
}
//using std::min;
