 ...
 int pad80(unsigned char *DataIn ,int *iDataLen)
{
	...
	if(*iDataLen<8)
	{
		...
		memset(DataIn+*iDataLen+1, 0x00, 8-byMod8);
		*iDataLen= 8;
	}
	else
	{
		...
		*iDataLen= *iDataLen+1;	//增加一个的长度
		...
		switch(byMod8)
		{
		case 0:	memset(DataIn+ byTempLen, 0, 7);  *iDataLen+= 7; break;
		case 1:	memset(DataIn+ byTempLen, 0, 6);  *iDataLen+= 6; break;
		case 2:	memset(DataIn+ byTempLen, 0, 5);  *iDataLen+= 5; break;
		case 3:	memset(DataIn+ byTempLen, 0, 4);  *iDataLen+= 4; break;
		case 4:	memset(DataIn+ byTempLen, 0, 3);  *iDataLen+= 3; break;
		case 5:	memset(DataIn+ byTempLen, 0, 2);  *iDataLen+= 2; break;
		case 6:	memset(DataIn+ byTempLen, 0, 1);  *iDataLen+= 1; break;
		}
	}
...
{
	unsigned char TempCommand[MAX_PATH]={0};
...
{
	int i=0, blklen=0;
	unsigned char bTemp[MAX_PATH];
	unsigned char temp1[MAX_PATH]={0};
	unsigned char temp2[MAX_PATH]={0};
